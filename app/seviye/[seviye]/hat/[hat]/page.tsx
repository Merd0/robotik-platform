  ...
  return (
    <main id="ana-icerik" data-seviye={level} className={`min-h-screen ${theme.page}`}>
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        <ol className="relative mt-10 space-y-4 before:absolute before:bottom-8 before:left-6 before:top-8 before:w-px before:bg-slate-300 sm:before:left-8">
          {lessons.map((lesson, index) => (
            <li key={lesson.slug} className="relative pl-14 sm:pl-20">
              <span aria-hidden="true" className="absolute left-0 top-6 z-10 grid size-12 place-items-center rounded-full border-4 border-white bg-slate-950 font-mono text-sm text-teal-300 sm:left-2">{index + 1}</span>
