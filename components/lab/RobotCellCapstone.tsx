  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <section className="lab-panel p-5 sm:p-6" aria-labelledby="gorev-paneli">
        <div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-teal-800">Çapraz-hat capstone</p><h2 id="gorev-paneli" className="mt-1 font-heading text-2xl font-semibold">{step + 1}. {STEPS[step]}</h2></div><span className="font-mono text-sm text-slate-500">{progress * 25}%</span></div>
        <ol className="mt-5 grid grid-cols-4 gap-1" aria-label="Görev ilerlemesi">{STEPS.map((label, index) => <li key={label}><button type="button" onClick={() => setStep(index)} className={`min-h-11 w-full rounded-lg text-xs ${step === index ? "bg-slate-950 text-white" : done[index] ? "bg-emerald-100 text-emerald-900" : "bg-slate-100 text-slate-500"}`} aria-label={`${index + 1}. ${label}${done[index] ? ", tamamlandı" : ""}`}>{done[index] ? "✓" : index + 1}</button></li>)}</ol>

        <div className="mt-6 min-h-72">
          {step === 0 && <div className="space-y-4"><p className="text-sm leading-6 text-slate-600">Kamerada fikstürün 120 px genişliği, gerçek hücrede 300 mm. Ölçeği bul.</p><label className="block text-sm font-medium">mm / piksel<input type="number" step="0.1" value={scale} onChange={(event) => setScale(Number(event.target.value))} className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3" /></label><button type="button" onClick={checkCalibration} className="min-h-11 w-full rounded-xl bg-slate-950 px-4 font-semibold text-white">Kalibrasyonu doğrula</button></div>}
          {step === 1 && <div className="space-y-4"><p className="text-sm leading-6 text-slate-600">Turuncu uçtan mavi kutuya giden iki aday yol var. Fikstürle çarpışmayanı seç.</p><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => checkRoute("duz")} className="min-h-11 rounded-xl border border-slate-300 px-3">Düz rota</button><button type="button" onClick={() => checkRoute("ust")} className="min-h-11 rounded-xl border border-slate-300 px-3">Üst rota</button></div></div>}
          {step === 2 && <div className="space-y-4"><p className="text-sm leading-6 text-slate-600">Komutları güvenli çalışma sırasıyla seç.</p><div className="grid gap-2">{PROGRAM.map(([id, label]) => <button key={id} type="button" disabled={program.includes(id)} onClick={() => addInstruction(id)} className="min-h-11 rounded-xl border border-slate-300 px-3 text-left text-sm disabled:opacity-40">{label}</button>)}</div><p className="min-h-8 rounded-lg bg-slate-100 p-2 font-mono text-xs">{program.length ? program.join(" → ") : "sıra boş"}</p><button type="button" onClick={resetProgram} className="min-h-11 text-sm underline underline-offset-4">Sırayı temizle</button></div>}
          {step === 3 && <div className="space-y-4"><p className="text-sm leading-6 text-slate-600">İnsan yaklaşırken robot hızını fiziksel modelin izin verdiği sınıra indir.</p><label className="block text-sm">Ayrım: {distance} mm<input type="range" min="300" max="2000" step="50" value={distance} onChange={(event) => setDistance(Number(event.target.value))} className="h-11 w-full touch-pan-y accent-teal-700" /></label><label className="block text-sm">Komut hızı: {speed} mm/s<input type="range" min="0" max="1600" step="50" value={speed} onChange={(event) => setSpeed(Number(event.target.value))} className="h-11 w-full touch-pan-y accent-teal-700" /></label><p className="rounded-xl bg-slate-100 p-3 text-sm">Model üst sınırı: <strong>{Math.round(allowed)} mm/s</strong></p><button type="button" onClick={checkSafety} className="min-h-11 w-full rounded-xl bg-slate-950 px-4 font-semibold text-white">Güvenliği doğrula</button></div>}
        </div>

        {progress === 4 && <div className="mt-4 rounded-2xl border border-emerald-300 bg-emerald-50 p-4"><strong className="text-emerald-950">Hücre devreye alındı.</strong><p className="mt-1 text-sm text-emerald-900">Dört hattaki kararların tek, sürümlü bir kanıta bağlandı.</p><button type="button" onClick={exportEvidence} className="mt-3 min-h-11 rounded-xl bg-emerald-900 px-4 text-sm font-semibold text-white">Kanıt JSON’unu indir</button></div>}
      </section>
    </div>
  );
}
