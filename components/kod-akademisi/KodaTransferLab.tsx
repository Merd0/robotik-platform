...
import { KODA_TRANSFER_SENARYOLARI } from "@/lib/kodaTransferGate";
import { useKodaTransferLab } from "./useKodaTransferLab";

export function KodaTransferLab() {
  const { code, setCode, durum, sonuclar, hataMesaji, runAll, reset } = useKodaTransferLab();

  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
        <div className="flex flex-col gap-3 rounded-xl border border-site-border bg-site-surface p-4">
          <span id="koda-transfer-editor-label" className="text-sm font-medium text-site-ink">
            Python kodu
          </span>
          <LazyPythonCodeEditor
            id="koda-transfer-editor"
            value={code}
            onChange={setCode}
            error={hataMesaji}
            labelledBy="koda-transfer-editor-label"
            tone="site"
          />
          ...
          {!sonuclar ? (
            ...
            <ul className="flex flex-col gap-2">
              {sonuclar.map((sonuc, index) => {
                const senaryo = KODA_TRANSFER_SENARYOLARI[index];
                return (
                  <li
                    key={sonuc.senaryoId}
                    data-testid={`koda-transfer-sonuc-${senaryo.id}`}
                  ...
                  >
                    ...
                    {sonuc.gecti && senaryo.gizli && (
                      <p className="mt-1 text-xs">Farklı koordinatlarla da doğru sonuca ulaştı — gerçekten genelledi.</p>
                    )}
                  </li>
                );
              })}
