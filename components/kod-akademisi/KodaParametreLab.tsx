...
import { KODA_PARAMETRE_SENARYOLARI } from "@/lib/kodaParametreTransfer";
import { useKodaParametreLab } from "./useKodaParametreLab";

export function KodaParametreLab() {
  const { code, setCode, durum, sonuclar, hataMesaji, runAll, reset } = useKodaParametreLab();

  return (
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
        <div className="flex flex-col gap-3 rounded-xl border border-site-border bg-site-surface p-4">
          <span id="koda-parametre-editor-label" className="text-sm font-medium text-site-ink">
            Python kodu
          </span>
          <LazyPythonCodeEditor
            id="koda-parametre-editor"
            value={code}
            onChange={setCode}
            error={hataMesaji}
            labelledBy="koda-parametre-editor-label"
            tone="site"
          />
          ...
          {!sonuclar ? (
            ...
            <ul className="flex flex-col gap-2">
              {sonuclar.map((sonuc, index) => {
                const senaryo = KODA_PARAMETRE_SENARYOLARI[index];
                return (
                  <li
                    key={sonuc.senaryoId}
                    data-testid={`koda-parametre-sonuc-${senaryo.id}`}
